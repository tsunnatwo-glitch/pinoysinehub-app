...
export const AddMovieForm: React.FC<AddMovieFormProps> = ({ onAddMovie, onUpdateMovie, editingMovie, isOpen, onClose, episodeMode = false }) => {
  ...
  const isEpisodeMode = Boolean(episodeMode && editingMovie?.type === 'series');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    let episodes = editingMovie?.episodes ? [...editingMovie.episodes] : undefined;

    if (isSeries) {
      // Huwag gumawa ng automatic episodes.
      // Kung may video URL, isang episode lang ang idaragdag/i-update.
      if (finalEmbed) {
        const safeEpisodeNumber = Math.max(1, Math.floor(episodeNumber || 1));

        const existingEpisodeIndex = episodes?.findIndex(
          (episode) => episode.episodeNumber === safeEpisodeNumber
        ) ?? -1;

        const episodeData = {
          id:
            existingEpisodeIndex >= 0
              ? episodes![existingEpisodeIndex].id
              : `ep-custom-${now}-${safeEpisodeNumber}`,
          episodeNumber: safeEpisodeNumber,
          seasonNumber:
            existingEpisodeIndex >= 0
              ? episodes![existingEpisodeIndex].seasonNumber || 1
              : 1,
          title:
            existingEpisodeIndex >= 0 && episodes![existingEpisodeIndex].title
              ? episodes![existingEpisodeIndex].title
              : `Kabanata ${safeEpisodeNumber}: ${title.trim()}`,
          duration:
            existingEpisodeIndex >= 0 && episodes![existingEpisodeIndex].duration
              ? episodes![existingEpisodeIndex].duration
              : '45m',
          description:
            existingEpisodeIndex >= 0 && episodes![existingEpisodeIndex].description
              ? episodes![existingEpisodeIndex].description
              : `Panoorin ang episode ${safeEpisodeNumber} ng ${title.trim()} sa PinoySineHub.`,
          thumbnail: '',
          videoUrl: finalEmbed,
          fileSizeMB:
            existingEpisodeIndex >= 0
              ? episodes![existingEpisodeIndex].fileSizeMB || 250
              : 250,
        };

        if (!episodes) {
          episodes = [episodeData];
        } else if (existingEpisodeIndex >= 0) {
          episodes[existingEpisodeIndex] = {
            ...episodes[existingEpisodeIndex],
            ...episodeData,
          };
        } else {
          episodes = [...episodes, episodeData].sort(
    ...
    const updatedMovie: Movie = {
      ...
      duration: isSeries
        ...
        : duration.trim() || '1h 45m',
      genres: [category, isSeries ? 'TV Series' : 'Pelikula', 'Tagalog Dubbed'],
      tags: [
        category,
        ...(editingMovie?.tags?.filter((tag) => tag !== 'Bagong Upload') || []),
        'Bagong Upload',
        'HD Streaming',
        'Tagalog Audio',
      ...
      type: contentType,
      category,
      // Series has no series-level video. Each episode has its own videoUrl.
      videoUrl: isSeries ? (editingMovie?.videoUrl || '') : finalEmbed,
      maturityRating,
      episodes,
    };
    ...
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 overflow-y-auto">
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={isSeries
                ? 'I-paste ang video embed para sa episode na idaragdag o ia-update...'
                : 'I-paste ang Abyss Embed URL o buong <iframe> code...'}
              value={videoUrl}
          ...
          {/* Series Episode Number or Movie Duration */}
          <div>
            {isSeries ? (
              <div>
                <label className="block text-neutral-300 font-bold mb-1.5">
